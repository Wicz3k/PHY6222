...
/*
    desc: chk mesg id
*/
int32 ppsp_impl_chk_msgs_numb(const void *mesg, uint16 coun)
{
    // logs_ent("mesg:#X%08x, coun:#d%d", mesg, coun);
    int32 rslt = FAILURE;
...
#if 1
    // pre-cfg value for debug only
    const uint8 valu[PPSP_IMPL_CFGS_ALIS_PIDS_COUN] = {
        0xe2,
        0x93,
        0x02,
        0x00,
...
#if 1
    // pre-cfg value for debug only
    const uint8 valu[PPSP_IMPL_CFGS_ALIS_MACS_COUN] = {
        0xf3,
        0xf2,
        0xf1,
        0xf0,
...
#if 1
    // pre-cfg value for debug only
    const uint8 valu[PPSP_IMPL_CFGS_ALIS_SCRT_COUN] = {
        0xd1,
        0xc6,
        0x53,
        0xb2,
...
int32 ppsp_impl_reg_serv_appl(ppsp_impl_clit_hdlr_t *clit_hdlr)
{
    ppsp_impl_set_auth_rslt(0);
    __ppsp_impl_msgs_numb = 0xff;
...
int32 ppsp_impl_reg_clit_appl(ppsp_impl_clit_hdlr_t *clit_hdlr)
{
    ppsp_impl_set_auth_rslt(0);
